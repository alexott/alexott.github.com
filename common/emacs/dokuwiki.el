...
(defun simple-dokuwiki-insert-headline (level)
  ...
  (interactive "nTiefe der Überschrift von 1 (oberste Ebene) bis 6: ")
...
If mark is active, use region as description."
  (interactive (list 
		...
		(if (simple-wiki-active-mark)
		  ...
		  (read-from-minibuffer "Beschreibung: "))))
...
(defun dokuwiki-login (&optional username password)
  ...
  (let* (success 
	 ...
	 ;;
	 (result 
	  (with-temp-message (format "Einloggen als Benutzer %s..." username)
	...
	(message "Konnte nicht einloggen.")
      (kill-buffer result))
    ;;
    success))
  ...
  (save-window-excursion
	...
	(when (bufferp buffer)
	  ...
	  (while (re-search-forward dokuwiki-index-re nil t)
	    (add-to-list 'indexlist (concat (match-string-no-properties 1) ":")))
	  (goto-char (point-min))
